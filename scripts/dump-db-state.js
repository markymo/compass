...
async function main() {
    ...
    // MasterFieldDefinitions (brief)
    const fieldCount = await prisma.masterFieldDefinition.count();
    console.log(`\n=== MASTER FIELD DEFINITIONS: ${fieldCount} total ===`);
    ...
    for (const cc of claimCounts) {
        if (!cc.subjectLeId) continue;
        const le = await prisma.legalEntity.findUnique({ where: { id: cc.subjectLeId } });
