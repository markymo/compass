...
function ExplorerRow({ row, tab, isSelected, isLast, onSelect, onAddToLibrary, onCreateWorkingCopy, onVisibility, onArchiveWC, onDeleteWC, onArchiveRef, onDeleteRef }: {
    ...
    return (
        <tr 
            ...
            className={cn("group cursor-pointer transition-colors focus-visible:bg-slate-100 focus-visible:outline-none", !isLast && "border-b border-slate-100", isSelected ? "bg-slate-50 border-l-2 border-l-slate-900" : "hover:bg-slate-50/60 border-l-2 border-l-transparent")}
        >
            {/* ── Name / Code ── */}
                ...
                <div className="flex items-start gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                            ...
                            {tab === "reference" && row.visibility && <VisibilityPill state={row.visibility} />}
                        </div>
                    </div>
                </div>
            </td>
...
function WCActions({ id, onAddToLibrary, onArchive, onDelete }: { id: string; onAddToLibrary: () => void; onArchive: () => void; onDelete: () => void }) {
    return (<>
        ...
        <button onClick={onArchive} className="p-1 rounded border border-slate-200 hover:border-amber-300 text-slate-400 hover:text-amber-600 transition-colors" title="Archive Working Copy" aria-label="Archive Working Copy"><Archive className="w-3 h-3" /></button>
        <button onClick={onDelete} className="p-1 rounded border border-slate-200 hover:border-red-300 text-slate-400 hover:text-red-500 transition-colors" title="Delete Working Copy" aria-label="Delete Working Copy"><Trash2 className="w-3 h-3" /></button>
    </>);
}
...
function RefActions({ id, row, onCreateWC, onVisibility, onArchive, onDelete }: { id: string; row: QV2Row; onCreateWC: () => void; onVisibility: () => void; onArchive: () => void; onDelete: () => void }) {
    ...
    return (<>
        ...
        <button onClick={onVisibility} title="Visibility" aria-label="Manage Visibility" className="text-[11px] text-slate-600 hover:text-slate-900 p-1 rounded border border-slate-200 hover:border-slate-300 transition-colors"><Eye className="w-3 h-3" /></button>
        <button onClick={onArchive} className="p-1 rounded border border-slate-200 hover:border-amber-300 text-slate-400 hover:text-amber-600 transition-colors" title="Archive Snapshot" aria-label="Archive Snapshot"><Archive className="w-3 h-3" /></button>
        <button
            onClick={canDelete ? onDelete : undefined}
            disabled={!canDelete}
            title={canDelete ? "Delete Snapshot" : "Cannot delete: this snapshot has been used to create other questionnaires"}
            aria-label={canDelete ? "Delete Snapshot" : "Cannot delete snapshot"}
            ...
            <div className="space-y-1.5 mt-auto">
                {isRef && row.isOnProOwned ? (<>
                    <Link href={`/app/admin/questionnaires/${row.id}`} className="flex items-center justify-between w-full text-xs font-medium text-slate-700 hover:text-slate-900 px-3 py-2 rounded border border-slate-200 hover:border-slate-300 transition-colors">
                        View questionnaire <ArrowRight className="w-3 h-3" />
                    </Link>
