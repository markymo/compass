...
describe("Registry Entity URLs Generic Resolver (ONP-37)", () => {
    describe("isSafeRegistryUrl", () => {
        ...
        it("rejects non-HTTPS or malformed URLs", () => {
            expect(isSafeRegistryUrl("http://insecure.example.com")).toBe(false);
    ...
    describe("getRegistryEntityUrl", () => {
        ...
        it("returns explicit entityUrl when already provided and valid", () => {
            const url = getRegistryEntityUrl({
                sourceType: "GLEIF",
                entityUrl: "https://search.gleif.org/#/record/CUSTOM_URL",
            });
            expect(url).toBe("https://search.gleif.org/#/record/CUSTOM_URL");
        });
