...
export async function generateSupersetWorkingCopy(
    ...
    try {
        // 1. Query all active Master Fields in canonical Master Record order + residual active fields
        const activeFields = await getSupersetOrderedMasterFields(prisma);
